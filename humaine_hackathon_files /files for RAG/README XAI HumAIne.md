...
### Local development / notebooks (recommended for contributors)

Install in editable mode from the repository root so notebooks can import
`explainerdashboard` without manual `sys.path` edits:

```bash
pip install -e .
```

The import path stays the same as upstream:

```python
from explainerdashboard import ClassifierExplainer, ExplainerDashboard
```

CLI entry points in this fork:

```bash
humaine-explainerdashboard --help
humaine-explainerhub --help
```
...
## HumAIne Additions vs Original `explainerdashboard`

This fork extends the upstream project with the following additions:

- **Weakspot Analysis**: identify underperforming data slices and inspect weak regions in feature space interactively (`WeakspotComponent` and explainer weakspot methods).
- **Plot Analysis (LLM-assisted)**: analyze dashboard plots or uploaded images with an LLM through `PlotAnalysisComponent`.
- **Text Explanation Component**: per-instance word-level text explanations with LIME via `TextExplanationComponent`.
- **Split-Conformalized Quantile Regression (Split-CQR)**: uncertainty-aware prediction intervals for regression explainers and dedicated visualization components.

Example notebooks for these additions:

- `notebooks/weakspot_analysis_example.ipynb`
- `notebooks/text_explanation_20newsgroups_demo.ipynb`
- `notebooks/split_conformalized_quantile_reg_example.ipynb`
